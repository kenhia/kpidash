...
static void gen_id(char *out, size_t len) {
    ...
    snprintf(out, len, "%lx-%lx", (unsigned long)time(NULL),
...
static bool queue_is_empty(void) { return g_count == 0; }
...
static void write_current_to_redis(redisContext *ctx, const status_message_t *m) {
    ...
    cJSON_AddStringToObject(root, "severity",
                            m->severity == STATUS_ERROR ? "error" : "warning");
    cJSON_AddStringToObject(root, "message", m->message);
    cJSON_AddNumberToObject(root, "ts", m->ts);
    char *json = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    if (json) {
        redisReply *r = redisCommand(ctx, "SET %s %s",
                                     KPIDASH_KEY_STATUS_CURRENT, json);
