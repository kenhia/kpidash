...
static int parse_int_env(const char *name, int default_val, int min_val, int max_val) {
    ...
    if (n < min_val || n > max_val) {
        fprintf(stderr, "config: %s=%s out of range [%d,%d], using default %d\n",
                name, v, min_val, max_val, default_val);
        return default_val;
    }
    return n;
}
...
void config_load(kpidash_config_t *cfg) {
    ...
    if (auth) strncpy(cfg->redis_auth, auth, sizeof(cfg->redis_auth) - 1);
    ...
    parse_str_env("KPIDASH_LOG_FILE", "/var/log/kpidash/kpidash.log",
    ...
    if (pc) {
        ...
        while (tok && cfg->priority_client_count < PRIORITY_CLIENTS_MAX) {
            /* trim leading spaces */
            while (*tok == ' ') tok++;
            strncpy(cfg->priority_clients[cfg->priority_client_count], tok,
                    HOSTNAME_LEN - 1);
