...
lv_obj_t *status_bar_create(lv_obj_t *parent) {
    ...
    lv_obj_set_style_bg_color(bar, WS_COLOR_BG, 0);
    lv_obj_set_style_radius(bar, 4, 0);
    lv_obj_set_style_border_width(bar, 1, 0);
    lv_obj_set_style_border_color(bar, WS_COLOR_SURFACE1, 0);
    ...
    lv_obj_set_style_text_color(lbl, WS_COLOR_YELLOW, 0);
    lv_obj_set_style_text_font(lbl, WS_FONT_TINY, 0);
...
void status_bar_show(lv_obj_t *bar, status_severity_t severity, const char *message) {
    ...
    if (lbl) {
        char buf[300];
        snprintf(buf, sizeof(buf), "[%s] %s", severity == STATUS_ERROR ? "ERROR" : "WARN", message);
        lv_label_set_text(lbl, buf);
        lv_color_t color = severity == STATUS_ERROR ? WS_COLOR_RED : WS_COLOR_YELLOW;
